...
interface SecurityEditorClientProps {
  initialSections: SecuritySection[]
}

export function SecurityEditorClient({ initialSections }: SecurityEditorClientProps) {
  const [sections, setSections] = useState<SecuritySection[]>(initialSections)
  const [saving, setSaving] = useState(false)

  const updateSection = (index: number, updates: Partial<SecuritySection>) => {
    setSections((prev) => {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const response = await fetch("/api/admin/security-sections", {
        method: "PUT",
      ...
      toast.success("Security page updated successfully!")
    } catch (error) {
        ...
        <Card key={section.id} className="p-6 mb-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold capitalize">{section.section_type.replace(/_/g, " ")} Section</h2>
              <div className="text-sm text-muted-foreground">Order: {section.display_order}</div>
            </div>
                ...
                {section.background_type === "gradient" || !section.background_type ? (
                  <div className="col-span-2">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Use hex color (#ffffff) or Tailwind gradient (from-blue-500 to-purple-600)
                    </p>
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateSection(index, { background_value: e.target.value })}
                    />
                    <p className="text-xs text-muted-foreground mt-1">Direct URL to an image file</p>
                  </div>
                ) : (
                  <div className="col-span-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      Paste the Blob URL for your video file (.mp4, .webm, .mov)
                    </p>
                  </div>
                )}
              </div>
