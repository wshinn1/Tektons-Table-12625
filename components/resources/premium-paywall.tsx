...
const benefits = [
  ...
  "150,000+ words of expert content",
  "In-depth fundraising strategies",
...
export function PremiumPaywall({ resource, isLoggedIn, categorySlug }: PremiumPaywallProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)

  const previewText = getPreviewText(resource.content)

  const handleSubscribe = async () => {
    if (!isLoggedIn) {
      router.push(`/auth/login?redirect=/resources/category/${categorySlug}/${resource.title}`)
      return
    }
    ...
    <div className="relative">
      ...
      <Card className="relative z-10 mt-8 border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-background overflow-hidden">
        ...
        <CardHeader className="text-center relative">
          ...
          <CardTitle className="text-2xl md:text-3xl mb-2">This is a Premium Resource</CardTitle>
          <p className="text-muted-foreground text-lg">Subscribe to unlock this and all other premium resources</p>
        </CardHeader>
